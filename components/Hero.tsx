...
const Hero = () => {
  useGSAP(() => {
    ...
    const initialTextSplit = new SplitText(".initialText", {
      type: "chars",
      mask: "chars",
    });
    const HeroTextSplit = new SplitText(".HeroName", {
      type: "chars",
      mask: "chars",
    });
    const paraTextSplit = new SplitText(".heroParagraph", {
      type: "words",
      mask: "words",
    });
    ...
    gsap.set(
      [paraTextSplit.words, initialTextSplit.chars, HeroTextSplit.chars],
      {
        yPercent: 100,
        rotateY: 70,
        xPercent: -20,
    ...
    <section className="hero h-screen space-y-8 font-outfit-medium flex flex-col justify-center">
      <div className="heroText uppercase font-mardon space-y-5">
        ...
        <div className="text-center space-y-5 md:space-y-0">
          <h1 className="HeroName text-[2.5rem] sm:text-[3.5rem] md:text-[6rem] lg:text-[8rem] text-primary-accent font-mardon tracking-widest leading-11 md:leading-normal will-change-transform">
            ABDERRAHMANE
          </h1>
          <p className="heroParagraph font-outfit-light lowercase text-[0.8rem] md:text-[1.3rem] leading-8 will-change-transform">
            Welcome to my lens where light, emotion, and creativity come to life
          </p>
        </div>
      </div>
