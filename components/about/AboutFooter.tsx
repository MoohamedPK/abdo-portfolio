...
const AboutFooter = ({ workMedia }: { workMedia: CloudinaryMediaProps[] }) => {
  ...
  return (
    <section
      ref={aboutFooterRef}
      className="aboutFooter h-[80dvh] md:h-[120dvh]"
    >
      <div className=" font-mardon py-20">
        <p
          className="quote text-center text-[1rem] sm:text-[2rem] lg:text-[2.5rem]
                max-w-250 mx-auto "
        >
          𓏲𝄢 DRIVEN BY PASSION AND DEFINED BY PERSPECTIVE 𓏲𝄢
        </p>
      </div>

      <div className="gallery flex md:flex-row flex-col justify-center overflow-auto">
        {workMedia.map((work) => (
          <div
            key={work.public_id}
