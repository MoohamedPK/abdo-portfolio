...
const AboutBody = () => {
  ...
  useGSAP(() => {
    ...
    // text split
    const aboutLableSplit = new SplitText(aboutLableRef.current, {
      type: "chars",
      mask: "chars",
    });
    const aboutParaSplit = new SplitText(aboutParaRef.current, {
      type: "lines",
      mask: "lines",
    });
  ...
  return (
    <section
      ref={aboutBodyRef}
      className="aboutSection bg-white py-20 flex flex-col justify-center "
    >
      <div className="flex flex-col md:flex-row justify-around items-center space-y-8 md:space-y-0 w-full">
        <div className="aboutTitle text-sm font-mardon text-primary-accent font-bold space-y-8 px-5 md:px-0">
          ...
          <div className="md:max-w-200 leading-6 text-xs md:text-sm text-black">
            <p ref={aboutParaRef} className="">
              The Hero Section Design, pro minently featured at the
              webpage&apos;s top, is crucial for capturing visitor attention and
              conveying the site&apos;s primary purpose. Designed for showcasing
              key product features, it can also promote special offers or
              actions.
            </p>
          </div>
        </div>
        ...
        <div className="aboutBodyImg">
          <Image
            src={"/about-img.jfif"}
            alt="about image"
            className="aboutImage rounded-t-full rounded-b-full object-cover max-h-130 max-w-90"
            width={350}
            height={350}
          />
        </div>
